...
\end{equation}
ニュートンの運動方程式における速度は、$\bm{R}$を微分することで得ることができる。我々は$\bm{R}$は知らないものの、$\bm{ｒ}$を知っている。
\begin{eqnarray}
...
　&=& \frac{\mathrm{d}}{\mathrm{d} t} \left\{ \bm{a} + \bm{b}(t) + \sum_{n=1}^{3}x_n (t) \bm{e}_n \right\} \nonumber \\
  &=& \bf{v} \nonumber
\end{eqnarray}
ところで、$\bm{a}$は一定であるので、式を整理して
\begin{equation}
  \frac{\mathrm{d}}{\mathrm{d} t} \bm{R} = \bf{v} = \frac{\mathrm{d}}{\mathrm{d} t} \bm{b} + \sum_{n=1}^{3} \frac{\mathrm{d}}{\mathrm{d} t}x_n \bm{e}_n
  \label{deｆｆ_of_R}
\end{equation}
を得ることができる。(ただし、これは$t$の関数である)\newline


\section{観測者の座標系の運動について考える}
宇宙上の物体を観測した観測者は沖縄にいるので、当然地球の自転によって運動している。だから、観測者の座標系が運動していることも検討しなければならない。
...
式(\ref{speed_of_observer_2})を、前に求めた式(\ref{deｆｆ_of_R})の第二項に代入して整理する。
